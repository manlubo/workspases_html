...
##### 탐색 선택자
- closest() : 부모들 중 첫번째 만나는 괄호안의 요소 선택
- parent() : 부모요소
- next(), prev() : 형제 중 다음/이전 첫번째 형제 선택
- nextAll(), prevAll() : 다음/이전 형제들 선택
- nextUntil(), prevUntil : 괄호 내부 전까지의 다음/이전 형제들 선택
- find() : 자식중 괄호안의 요소 선택

- add() : 추가로 하나 더 선택
- end() : 실행이 끝나면 이전 선택자로 돌아갈 수 있음
- is() : 괄호 내부에 조건 ~ 하냐? boolean type

### 메서드
##### 배열관련
- .each(i, j) : 내부요소 탐색 i : 인덱스, j : 값

##### 요소 메서드
- .text() : 괄호 내부의 문자를 넣거나, 그냥 사용 시 내부의 텍스트가져옴
- .html() : 괄호 내부의 html를 넣거나, 그냥 사용 시 내부의 html가져옴

##### 클래스 메서드
- .addClass() : 요소에 괄호안의 클래스를 추가
- .removeClass() : 요소에 괄호안의 클래스를 제거
- .hasClass() : 괄호안의 클래스가 있는가? boolean
